...
| addressIdx    | int64  | 否   | 系统钱包地址索引（与 [生成地址](#生成地址) 的 `addressIdx` / `accountID` 同义）。**仅当需要将客户收款地址写入监控时传入**；须为本商户已生成的地址索引，否则返回 `addressIdx not found`。**未传或为 0 时不创建提现监控记录** |
...
- 若商户已开启客户地址监控，且申请提现时传入了 `addressIdx`，链上确认后会将 `toAddr` 写入监控并关联对应系统钱包；**未传 `addressIdx` 则不会因本次提现新增监控记录**（详见 [客户地址监控（Tron）](README.md#客户地址监控tron)）。
