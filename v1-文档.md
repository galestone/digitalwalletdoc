...
| addressIdx    | int64  | 否   | 系统钱包地址索引（与 [生成地址](#生成地址) 的 `addressIdx` / `accountID` 同义）；用于关联客户地址监控。须为本商户已生成的地址索引，否则返回 `addressIdx not found`。未传时，链上确认后 daemon 会从出款地址反查 accountID |
...
- 若商户已开启客户地址监控，提现链上确认后会将 `toAddr` 写入监控；传入 `addressIdx` 可指定关联的系统钱包，便于按钱包维度查询客户地址变化（详见 [客户地址监控（Tron）](README.md#客户地址监控tron)）。
