...
查询当前商户在指定链、指定代币下的**提现地址**余额（代币余额与 gas 费余额）。用于提现前确认可用额度。默认返回缓存数据；传 `syncFromChain=true` 时从链上拉取最新余额并回写缓存。
...
| syncFromChain  | bool   | 否   | 为 `true` 时从链上同步最新余额并更新缓存，再返回；不传或为 `false` 时仅返回缓存值 |
...
**说明：** 需在后台为该商户、该链/合约配置并开启提现配置，否则返回「withdraw config not found」。`contractAddr` 格式错误会返回「contractAddr is invalid」。
