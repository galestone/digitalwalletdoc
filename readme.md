...
- [数字钱包平台 - 第三方接入文档](#数字钱包平台---第三方接入文档)
  ...
  - [TRON 相关接口](#tron-相关接口)
    ...
    - [提现](#提现)
      ...
      - [按 memo 取消提现（TRON）](#按-memo-取消提现tron)
      - [提现记录列表（分页）](#提现记录列表分页)
  - [ETH 相关接口](#eth-相关接口)
    - [地址管理](#地址管理-1)
      - [生成地址](#生成地址-1)
...
| memo          | string | 否   | 备注，会随提现回调带给接入方，并可用于 [按 memo 取消提现（TRON）](#按-memo-取消提现tron) |

**响应 data：**

| 字段 | 类型   | 说明 |
|------|--------|------|
| id   | string | 提现记录 ID，用于 [链上取消提现（按 ID）](#链上取消提现按-id) |
| memo | string | 与请求一致的备注，用于 [按 memo 取消提现（TRON）](#按-memo-取消提现tron) |

成功仅表示已入库，实际到账以回调为准。

**说明：**

- 提现前需在后台为该商户、该合约配置提现参数（最小/最大金额、确认数等），否则会报「提现配置不存在」或「提现配置已禁用」。
- `amount` 需大于 0，且不超过后台配置的 `maxWithdrawAmount`（若已配置）；低于最小提现金额会报「提现金额小于最小提现金额」。
- 若提现地址代币或 TRX（gas）余额不足，会返回相应错误（如「提现地址代币余额不足」「提现地址TRX余额不足，无法支付gas」）。
...
按提现记录 ID 取消一笔尚未广播到链上的提现申请。仅当提现状态为「已入库等待广播」时可取消；一旦已广播则不可取消（会返回「withdraw already broadcast, cannot cancel」）。
...
**响应 data：** 空对象 `{}`。成功表示已取消，该笔提现不会上链。若记录不存在或非本商户则返回「withdraw not found」。

---

#### 按 memo 取消提现（TRON）

**POST** `/v1/tron/withdraw/cancelbymemo`
...
**响应 data：** 空对象 `{}`。若未找到对应提现则返回「withdraw not found」。
...
| memo          | string | 否   | 备注，会随提现回调带给接入方，并可用于 [按 memo 取消提现（ETH）](#按-memo-取消提现eth) |

**响应 data：**

| 字段 | 类型   | 说明 |
|------|--------|------|
| id   | string | 提现记录 ID，用于 [链上取消提现（按 ID）](#链上取消提现按-id-1) 或 [提现记录列表（分页）（ETH）](#提现记录列表分页eth) 查询 |
