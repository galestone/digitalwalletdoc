...
1. 平台会给后台用户，[点我登录测试后台](http://154.82.113.141:12583/login)，登录后台创建商户，配置充值/提现回调 URL，添加后会生成 **API Key** 和 **Signature Key**。
![新增商户](./新增商户.png)
2. 拿到 **API Key** 后，就可以调用api接口了。
3. 充值流程
   1. 为用户生成充值地址（一个用户生成一次就行）
   2. 用户拿到地址后，可以要随时充值。
...
**响应 data：** 空对象 `{}`。成功仅表示已入库，实际到账以回调为准。
...
| amount              | string | 提现数量 |
| memo                | string | 备注 |
| status              | int    | 状态：1=已入库等待广播，2=已广播等待确认，3=已完成，4=链上失败，5=未知 |
